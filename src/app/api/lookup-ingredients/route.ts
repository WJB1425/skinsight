...
const NORMALIZE_PROMPT = `用户给一段化妆品成分表文字（可能是日语 / 法语 / 英语等任意语言，或含编号、杂质、断行）。把它翻译并规范化为【中文规范成分名】列表。
规则：
1. 用中文顿号「、」分隔，保持原有先后顺序。
2. 只输出成分名本身，去掉编号 / 小标题（如「全成分」「Ingredients」「表面活性剂」）/ 无关说明。
3. 不要新增或编造成分，不要漏掉成分；拿不准的成分保留其原文。
4. 严格只输出 JSON：{"ingredients":"成分1、成分2、…"}。`;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    normalize = String(body?.normalize ?? '').trim();
  } catch {
    /* ignore bad body */
  }
  if (!name && !image && !suggest && !normalize) {
    return NextResponse.json({ configured: true, found: false, error: '请提供产品名称、图片、搜索词或成分表文本' });
  }
  ...
  try {
    ...
    if (normalize) {
      const res = await callLLM(base, apiKey, textModel, [
        ...
        { role: 'user', content: normalize.slice(0, 4000) },
      ]);
      if (!res.ok) return NextResponse.json({ configured: true, found: false, error: `模型调用失败 (${res.status})` });
      const parsed = extractJson(res.content);
      const ingredientsText = parsed?.ingredients ? String(parsed.ingredients) : '';
      return NextResponse.json({ configured: true, found: Boolean(ingredientsText), ingredientsText });
    }
