...
export function DataSource({ className }: { className?: string }) {
  return (
    <details className={cn('group rounded-xl border border-border bg-surface-hover/60', className)}>
      <summary className="flex items-center justify-between gap-2 cursor-pointer list-none px-3 py-2.5 text-xs font-medium text-muted hover:text-foreground [&::-webkit-details-marker]:hidden">
      ...
      <div className="px-3 pb-3 pt-1 text-xs text-muted leading-relaxed space-y-2">
        ...
        <p>
          <span className="text-emerald-600 font-medium">✓ 可验证：</span>
        ...
        <p>
          <span className="text-amber-600 font-medium">参考分级：</span>
